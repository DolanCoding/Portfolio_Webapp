...
// 🔒 INVARIANTS: DB_PATH resolves to a readable SQLite file; defaults under ./db/portfolio.db (same folder as this file)
const DEFAULT_DB_PATH = path.resolve(__dirname, "./db/portfolio.db");
const dbPathEnv = process.env.DB_PATH;
const RESOLVED_DB_PATH = dbPathEnv ? path.resolve(dbPathEnv) : DEFAULT_DB_PATH;
...
async function openDatabase(): Promise<void> {
  ...
  try {
    db = await open({
      filename: RESOLVED_DB_PATH,
      driver: sqlite3.Database,
    });
    console.log(`Opened main DB connection at: ${RESOLVED_DB_PATH}`);
  } catch (e: unknown) {
    console.error("Error opening database:", e, "(Attempted path:", RESOLVED_DB_PATH, ")");
    process.exit(1);
  }
}
